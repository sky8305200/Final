...
chat.userMsg = function(socket) {
	...
	socket.on('chat message', function(msg){
		msg = that.userName[socket.id] + ' 說: ' + msg;
...
chat.assignGuestName = function(socket) {
	...
	var msg = this.userName[socket.id] + ' 加入房間! Welcome!';
...
chat.disconnect = function(socket) {
	...
	socket.on('disconnect', function(){
		var msg = that.userName[socket.id] + ' 離開';
...
chat.changeName = function(socket) {
	...
	socket.on('change name', function(msg){
		if (that.usedName.indexOf(msg) == -1) {
			...
			socket.emit('sys message', ' 你的暱稱改為 ' + msg);
		}
		else {
			socket.emit('sys message', '此暱稱已被使用');
		}
...
chat.assignRoom = function(socket) {
	...
	socket.join('TEST', function(){
		that.currentRoom[socket.id] = 'TEST';
	});
}
...
chat.changeRoom = function(socket) {
	...
	socket.on('change room', function(msg){
		...
		var sysMsg = that.userName[socket.id] + ' 離開 ' + '房間:' + that.currentRoom[socket.id];
		...
		socket.leave(that.currentRoom[socket.id], function(){
			...
			sysMsg = that.userName[socket.id] + ' 加入 ' + '房間:' + that.currentRoom[socket.id];
